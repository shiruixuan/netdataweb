...
netdataDashboard.menu = {
    ...
    'ipv4': {
        title: 'IPv4 联网状态',
        icon: '<i class="fas fa-cloud"></i>',
        info: '系统的 IPv4 协议堆栈指标。 ' +
            '<a href="https://en.wikipedia.org/wiki/IPv4" target="_blank">' +
            'Internet Protocol version 4 (IPv4)</a>' +
            '  是互联网协议Internet Protocol (IP) 的第四个版本。' +
            '它是互联网中基于标准的网络互联方法的核心协议之一。' +
            'IPv4 是一种用于分组交换网络的无连接协议。' +
            '它采用尽力而为的交付模式，即它不保证交付，也不确保正确的顺序或避免重复交付。' +
            '这些方面（包括数据完整性）由上层传输协议来处理，如传输控制协议Transmission Control Protocol (TCP)。'
    },
    ...
    'disk': {
        ...
        info: '包含所有系统磁盘性能信息的图表。 已经以兼容的方式呈现磁盘性能指标 <code>iostat -x</code>. netdata默认情况下会阻止单个分区和卸载的虚拟磁盘的呈现性能图表。仍然可以通过配置netdata配置文件中的相对设置来启用已禁用的图表。'
    },
    ...
    'apps': {
        ...
        ' 插件获取每个应用程序的统计数据。该插件会遍历所有流程，并汇总'+
        '<a href="https://learn.netdata.cloud/docs/agent/collectors/apps.plugin#configuration" target="_blank">'+
        '应用程序组</a>的统计数据。该插件还会统计已退出子进程的资源情况。'+
        '因此，对于诸如 shell 脚本之类的进程，所报告的值包含'+
        '这些脚本在每个时间范围内运行的命令所使用的资源。',
        height: 1.5
    },
...
netdataDashboard.submenu = {
    ...
    'cpu.softnet_stat': {
        ...
        info: function (os) {
            if (os === 'linux')
                return '<p>与CPU接收工作相关的每CPU核心SoftIRQ的统计信息。'+
...
netdataDashboard.context = {
    'system.cpu': {
        info: function (os) {
            void (os);
            return 'CPU使用率（所有核心）。这里100% 表示已经没有空闲CPU资源可用。你可以从 <a href="#menu_cpu">CPU 总</a> 获取每个核心的信息，从<a href="#menu_apps">应用监控</a>获取每个应用的信息。'
                + netdataDashboard.sparkline('<br/>请关注 <b>iowait</b> ', 'system.cpu', 'iowait', '%', '。如果它一直很高，那么您的磁盘就会成为瓶颈，它们会降低系统速度。')
                + netdataDashboard.sparkline(
                '<br/>还有一个值得监控的重要指标是：<b>softirq</b> ',
                'system.cpu',
                'softirq',
                '%',
                '。持续高百分比的softirq可能表示网络驱动程序问题。'+
                '各个指标可在'+
                '<a href="https://www.kernel.org/doc/html/latest/filesystems/proc.html#miscellaneous-kernel-statistics-in-proc-stat" target="_blank">内核文档</a>中找到。');
        },
        valueRange: "[0, 100]"
    },

    'system.load': {
        info: '当前系统负载。 使用CPU或等待系统资源的进程数(通常是CPU和磁盘). 3个指标指的是1分钟，5分钟和15分钟的平均值. 系统每5秒计算一次。点击<a href="https://en.wikipedia.org/wiki/Load_(computing)" target="_blank">维基百科</a>查看更多信息',
        height: 0.7
    },
    ...
    'system.io': {
        info: function (os) {
            var s = '所有物理磁盘 I/O。点击 <a href="#menu_disk">Disks</a> 查看每个磁盘的详尽信息。点击 <a href="#menu_apps">应用监控</a> 查看每个应用的详尽信息';

            if (os === 'linux')
                return s + ' 所有在 <code>/sys/block</code>列出来的物理磁盘, 并不存在于 <code>/sys/devices/virtual/block</code>.';
            else
                return s;
        }
    },
    ...
    'system.intr': {
        colors: '#DD5555',
        info: 'CPU中断总数。点击 <code>system.interrupts</code> 将会看到更多信息，也可以点击 <a href="#menu_cpu">CPUs</a> 查看每个CPU的中断情况'
    },

    'system.interrupts': {
        info: 'CPU中断详情。在 <a href="#menu_cpu">CPU 总</a> 面板, 可以分析 <a href="#menu_cpu_submenu_interrupts">每个 CPU</a> 的情况。'+
        '<code>/proc/interrupts</code> 文件中的最后一列提供了中断描述或者为该中断注册了处理程序的设备名称。'
    },
    ...
    'system.processes': {
        info: '<p>系统进程。</p>'+
        '<p><b>Running</b> 是运行在CPU的进程，'+
        '<b>Blocked</b> 是正在等待进入CPU的进程, 因为他们在等待磁盘IO。</p>'
    },
    ...
    'system.ctxt': {
        info: '<a href="https://en.wikipedia.org/wiki/Context_switch" target="_blank">Context Switches</a>是CPU从一个进程、任务切换到另外一个进程任务. 如果很多进程和线程要使用有限的CPU执行，系统将会通过context switching 来平衡CPU资源分配。若进程是计算密集型，将会进行更多的context switching，这将会拖慢系统运行速度。'
    },

    'system.idlejitter': {
        info: '空闲抖动由netdata计算。 产生一个请求睡眠几微秒的线程。 当系统唤醒它时，它会测量已经过了多少微秒。 The difference between the requested and the actual duration of the sleep, is the <b>idle jitter</b>. This number is useful in real-time environments, where CPU jitter can affect the quality of the service (like VoIP media gateways).'
    },

    'system.net': {
        info: function (os) {
            var s = '所有物理网络接口的总带宽。。 这并不包括 <code>lo（虚拟网络）</code>, VPNs（代理）, 网络桥接,监听设备, 网卡绑定等. 仅聚合物理网络接口的带宽。';

            if (os === 'linux')
                return s + ' 物理网卡只包含<code>/proc/net/dev</code>列出来的, 并不包含<code>/sys/devices/virtual/net</code>.';
            else
                return s;
        }
    },
    ...
    'system.ram': {
        info: '系统随机存取存储器（物理存储器）的使用'
    },

    'mem.swap': {
        info: '系统交换内存使用量。物理内存（RAM）已满时使用交换空间。当系统需要更多内存单RAM已满时，不活跃的页面将会被移至交换内存中（通常是磁盘、磁盘分区或者文件形式）。'
    },
    ...
    'mem.pgfaults': {
        info: '<p><a href="https://en.wikipedia.org/wiki/Page_fault" target="_blank">页面错误</a>'+
        '是当一个正在运行的程序访问映射到虚拟地址空间但实际上没有加载到主存储器中的内存页时，'+
        '由计算机硬件引发的一种称为陷阱的中断。</p>'+
        '<p>如果在生成故障时将页面加载到内存中，但未在内存管理单元中将其标记为已加载到内存中，'+
        '则称其为 <b>Minor</b> 或软页面错误。'+
        '当系统需要从磁盘或交换内存加载内存页时，会产生 <b>Major</b> 错误。</p>'
    },

    'mem.committed': {
        colors: NETDATA.colors[3],
        info: 'Committed Memory，是进程分配的所有内存的总和。'
    },
    ...
    'mem.available': {
        info: function (os) {
            if (os === "freebsd")
                return '用户空间进程在不引发交换（内存换出到磁盘）的情况下能够使用的内存量。其计算方式为空闲内存、缓存内存以及非活跃内存之和。';
            else
                return '可用内存由内核估算，作为用户空间进程可以使用的RAM量，而不会导致交换。';
        }
    },
    ...
    'mem.kernel': {
        info: '<p>内核使用的内存总量。</p>'+
        '<p><b>Slab</b> 是内核用于缓存数据结构以供自己使用的内存量。'+
        '<b>KernelStack</b> 是为内核完成的每个任务分配的内存量。'+
        '<b>PageTables</b> 是专用于最低级别页表的内存量（页表用于将虚拟地址转换为物理内存地址）。'+
        '<b>VmallocUsed</b> 是用作虚拟地址空间的内存量。'+
        '<b>Percpu</b> 是分配给用于支持每 CPU 分配的每 CPU 分配器（不包括元数据开销）。'+
        '当你创建一个每 CPU（基于每个 CPU 的）变量时，系统上的每个处理器都会拥有该变量的专属副本。</p>'
    },
    ...
    'ip.inerrors': {
        info: '<p>接收IP数据包时遇到的错误.。</p>' +
            '</p><b>NoRoutes</b> 是计算丢弃的数据包，因为没有路由发送它们。' +
            '<b>Truncated</b> 是计数被丢弃的数据包，因为数据报帧没有携带足够的数据。' +
            '<b>Checksum</b> 是计算由于校验和错误而丢弃的数据包. </p>'
    },
    ...
    'ipv4.tcpsock': {
        info: '当前状态为“已建立（ESTABLISHED）”或“等待关闭（CLOSE-WAIT）”的 TCP 连接的数量。'+
        '这是测量时建立的连接的快照'+
        '（即建立的连接和在同一次迭代中断开的连接不会影响该度量）。'
    },
    ...
    'apps.vmem': {
        info: '虚拟内存分配。'+
        '点击 <a href="https://github.com/netdata/netdata/tree/master/src/daemon#virtual-memory" target="_blank">这篇文章</a> 查看更多信息。'
    },
    'groups.vmem': {
   ...
   // Apps eBPF stuff
    ...
    'disk.inodes': {
        info: 'Inodes（或索引节点 index nodes）是文件系统对象（例如文件和目录）。在许多类型的文件系统实现中，最大数量的 inodes 在创建文件系统时是固定的，限制了文件系统可以容纳的最大文件数。 设备可能耗尽 inodes。 发生这种情况时，即使可能有可用空间，也无法在设备上创建新文件。'
    },
